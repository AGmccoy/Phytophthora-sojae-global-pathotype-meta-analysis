---
title: "renv environment for meta analysis"
author: "Austin McCoy"
date: "1/25/2023"
...
#Loading in the specific pakcage versions needed for reproducible analysis. Format = renv::install("PACKAGE@V.ERS.ION")
```{r}
#renv::install("openxlsx@4.2.4")
#renv::install("plyr@1.8.6")
#renv::install("adegenet@2.1.4")
#renv::install("tidyverse@1.3.1")
#renv::install("DT@0.19")
#renv::install("hagis@3.1.4")
#renv::install("pander@0.6.4")
#renv::install("vegan@2.4-3")
#renv::install("ape@5.6-2") 
#renv::install("ggsigniff@0.6.3") installed with ggpubr, not needed here
#renv::install("cowplot@1.1.1")
#renv::install("ellipse@0.4.2")
#renv::install("BiocManager")
#renv::settings$bioconductor.version("3.13")
#BiocManager::install('mixOmics') # If something besides the permanova bugs out now, its likely this package. lots of dependables. maybe look into if we can find another version closer to RVAide
#BiocManager::install("BiocVersion")
#renv::install("RVAideMemoire@0.9-81-2") # not installing due to mixomics
#renv::install("ggpubr@0.4.0")
#renv::install("here@1.0.1")
#renv::install("sf@1.0-2")
#renv::install("readxl@1.3.1")


#renv::snapshot() #saving our package versions to renv.LOCK
...
###### Packages (R version: 4.1.1) needed for data exploration and analysis. version indicates the versions that we loaded
...
```{r}
t_USA_vir_jaccard <- t(USA_vir_jaccard)

USA_patho_perma <- adonis(unname(t_USA_vir_jaccard) ~ USA_groups)

USA_patho_perma

USA_patho_pair_perma <- pairwise.perm.manova(as.matrix(t_USA_vir_jaccard), USA_groups, nperm = 999, progress = TRUE, R2=TRUE)
...
```{r}
t_ARG_vir_jaccard <- t(ARG_vir_jaccard)

ARG_patho_perma <- adonis(unname(t_ARG_vir_jaccard) ~ ARG_groups)

ARG_patho_perma

ARG_patho_pair_perma <- pairwise.perm.manova(unname(t_ARG_vir_jaccard), ARG_groups, nperm = 999, R2=TRUE)
...
```{r}
t_CAN_vir_jaccard <- t(CAN_vir_jaccard)
CAN_patho_perma <- adonis(unname(t_CAN_vir_jaccard) ~ CAN_groups)

CAN_patho_perma

CAN_patho_pair_perma <- pairwise.perm.manova(unname(t_CAN_vir_jaccard), CAN_groups, nperm = 999, R2=TRUE)
...
```{r}
t_CHI_vir_jaccard <- t(CHI_vir_jaccard)
CHI_patho_perma <- adonis(unname(t_CHI_vir_jaccard) ~ CHI_groups)

CHI_patho_perma

CHI_patho_pair_perma <- pairwise.perm.manova(unname(t_CHI_vir_jaccard), CHI_groups, nperm = 999, R2=TRUE)
